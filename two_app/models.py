...
def state_from_request(cls, request):
    ...
    try:
        ...
        print('user_type is not exist')

    try:
        request.session["shop_name"] = get_request_param(request, "shop_name", None)
    except KeyError:
        print('shop_name is not exist')

    try:
        request.session["shop_id"] = get_request_param(request, "shop_id", None)
    except KeyError:
        print('shop_id is not exist')
...
class Shop(models.Model):
    ...
    shop_name = models.SlugField(max_length=255, blank=True)
    shopId = models.UUIDField(
        default=uuid4, editable=True, unique=True, blank=False)
